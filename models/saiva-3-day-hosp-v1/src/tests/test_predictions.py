...
# python /src/run_model.py --client trio --facilityids '[186]' --cache_strategy ignore_cache
#         --prediction_date 2021-02-15 --s3_bucket saiva-dev-data-bucket
...
def cmd_run_model(client, facility_id, prediction_date, cache_strategy, 
    ...
    cmd = (f"python /src/run_model.py --client {client} " + 
           ...
           f"--local_folder {local_folder_path} --cleanup_downloaded_models False")
...
@pytest.fixture(scope="module", params=[('trio', '186', '2021-02-15', 'golden/run1',
                                         '/data/trio-test/2021-02-15'),
                                         ('marquis', '7', '2021-02-15', 'golden/run1',
                                         '/data/marquis-test/2021-02-15')])
...
def dfs_are_equal(df1, df2):
    print(f"shape of df1: {df1.shape}; shape of df2: {df2.shape}")
    df = pd.concat([df1, df2]).drop_duplicates(keep=False)
    print(f"shape of duplicate df: {df.shape}")
    if len(df) != 0:
...
def strs_are_equal(golden_str, local_str):
    ...
    print(f"length of normalized golden str: {len(golden)}; length of normalized local str: {len(local)}")
    # print(f"golden: {g1}")
    # print(f"local: {l1}")
    if golden != local:
...
def test_queries_are_same(run_model, query_name):
    ...
    try:
        ...
        if ex.response['Error']['Code'] == 'NoSuchKey' and query_name == "patient_lab_results":
            print("cannot find patient_lab_results query: ignoring exception - client may not contain lab_results")
        else:
            raise
...
def test_predictions_are_consistent(request):
    ...
    run1_local_path = '/data/test/2021-02-15/run1'
    run2_local_path = '/data/test/2021-02-15/run2'
    dryrun = request.config.getoption("--dryrun")
    if not dryrun:
        ...
        cmd_run_model(client = 'trio',
                    facility_id = '186',
                    prediction_date = '2021-02-15',
