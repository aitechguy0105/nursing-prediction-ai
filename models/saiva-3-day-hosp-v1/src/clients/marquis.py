...
class Marquis(BaseClient):
    ...
    def get_training_dates(self):
        return '2018-07-01', '2020-10-31'

    def validate_dataset(self, facilityid, dataset_name, dataset_len):
        if (dataset_name == 'patient_lab_results') and (facilityid in [2, 4, 5, 7, 13, 15, 18, 34, 36, 40, 42]):
            # these facility ids for marquis don't have labs
            pass
        elif (dataset_name in ['patient_meds', 'patient_alerts', 'patient_orders']) and (facilityid in [36]):
            # special case added for marquis facility 36
            pass
        elif (dataset_name in ['patient_alerts']) and (facilityid in [40]):
            # special case added for marquis facility 40
            pass
        else:
    ...
    def get_excluded_censusactioncodes(self):
        ...
        """
        return ['DD', 'DE', 'TO', 'RDD', 'RDE', 'RDH', 'DH', 'HL',
