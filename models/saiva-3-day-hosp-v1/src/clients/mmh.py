...
class Mmh(BaseClient):
    ...
    def get_training_queries(self, test_end_date, train_start_date):
        ...
        return {
            'patient_vitals': f"""
                        ...
                        and facilityid in (select facilityid from view_ods_facility where lineofbusiness in ('SNF', 'WLC', 'ABU'))
            ...
            'master_patient_lookup': f"""
                        ...
                        where facilityid in (select facilityid from view_ods_facility where lineofbusiness in ('SNF', 'WLC', 'ABU'))
                        """,
            'patient_census': f"""
                        ...
                        and censusactioncode not in ('DD', 'DE', 'TO', 'RDD', 'RDE', 'RDH', 'DH', 'HP', 'TP', 'L', 
    ...
    def get_note_embeddings_emar_types(self):
        return ['eMar - Hour of Administration Note']

    def get_note_embeddings_nan_threshold(self):
        return 0.30

    def get_training_dates(self):
        return '2018-01-01', '2020-11-30'

    def validate_dataset(self, facilityid, dataset_name, dataset_len):
        if (dataset_name == 'patient_lab_results') and (facilityid != 1):
            pass
        else:
            # There should be data in the dataset otherwise
    ...
    def get_excluded_censusactioncodes(self):
        ...
        """
        return ['DD', 'DE', 'TO', 'RDD', 'RDE', 'RDH', 'DH', 'HP', 'TP', 'L',
                'DAMA', 'LSNF', 'TOSNF', 'HUP', 'TUP', 'HL', 'HUM']
