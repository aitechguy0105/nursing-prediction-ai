"""
- patient_room_details query is different from other clients
- Trio is still on V3 etl
"""
import sys
...
class Trio(BaseClient):
    ...
    def get_prediction_queries(self, prediction_date, facilityid, train_start_date):
        ...
        return {
            ...
            'patient_room_details': f"""
                            select c.patientid,fp.masterpatientid, c.facilityid, 
                            c.censusdate, fp.initialadmissiondate, c.payername, 
                            b.Floor as floor, b.FloorID as floor_id, u.UnitDescription as unit, u.UnitID as unit_id 
                            from dbo.view_ods_daily_census_v2 c 
                            JOIN dbo.view_ods_facility_patient fp on (fp.facilityid = c.facilityid and fp.patientid = c.patientid)
    ...
    def get_excluded_censusactioncodes(self):
        ...
        E(Expired), DAMA(Discharge AMA), DD(Dischargedate)
        """
        return ['DAMA', 'DD', 'DE', 'DH', 'E', 'HU', 'L', 'LV', 'MO', 'TO', 'TP', 'TU']
