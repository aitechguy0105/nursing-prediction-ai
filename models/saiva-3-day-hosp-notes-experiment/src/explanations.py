...
def process_attributions(numeric_attributions):
    ...
    type_mapping_dict = {
        ...
        r'^cumsum_all_note__.*': 'Progress Note',
         r'^cumsum_\d+_daynote__.*': 'Progress Note',
        r'^topic-.*': 'Progress Note',
...
def alert_mapper(alert_attribution, alert_data, mpid_to_use, date_to_use):
    human_readable_name = ''
    mapping_status = 'NOT_MAPPED'
    if alert_attribution['mapped_feature'].startswith('cumsum_alert_')and (alert_attribution['feature_value'] == 0):
        # if feature_value is 0 for cumsum features, it means that no alerts exist for that med feature
        mapping_status = 'NOT_RELEVANT'
    elif alert_attribution['mapped_feature'].startswith('cumsum_alert_') and '_count_' not in alert_attribution['mapped_feature']:
        alert_name = alert_attribution['mapped_feature'].replace('cumsum_alert_count_indicator_', '').replace('cumsum_alert_indicator_', '').replace('cumsum_alert_', '')
        ...
        if len(alert_reason) > 0:
            ...
            if day_count in exp_dictionary['Alert_Indicator'].keys() and \
                ...
                mapping_status = 'DATA_FOUND' # change to data mapped in future
            else:
                mapping_status = 'DATA_FOUND'
            
        else:
            human_readable_name += f"{int(alert_attribution['feature_value'])} alerts for '{alert_name}' in last {day_count} days"
            mapping_status = 'DATA_FOUND'
        ...
        if 'na_indictator_alert_indicator_' in alert_attribution['mapped_feature']:
            alert_type = alert_attribution['mapped_feature'].replace('na_indictator_alert_indicator_', '').replace('na_indictator_alert_', '')
...
def med_mapper(med_attribution, med_data, mpid_to_use, date_to_use):
    ...
    if (med_attribution['mapped_feature'].startswith('cumsum_med_')) and (med_attribution['feature_value'] == 0):
        ...
        if (len(med_reason) > 0):
            ...
            human_readable_name = f"{med_name} ordered on {med_reason.iloc[0]['orderdate']}." 
...
def vital_mapper(vital_attribution, vital_data, mpid_to_use, date_to_use):
...
def notes_mapper(notes_attribution, mpid_to_use, date_to_use):
    cumsum_match = re.match(r'^cumsum_(.*)note__(.*)$', notes_attribution['mapped_feature'])
    topic_match = re.match(r'^topic-(.*)$', notes_attribution['mapped_feature'])
    human_readable_name = ''
    mapping_status = 'NOT_MAPPED'
    if notes_attribution['feature_value']>0:
        mapping_status = 'DATA_FOUND'
        if cumsum_match:
            cumsum_keyword = cumsum_match.groups()[1]
            day_count = get_day_count(notes_attribution)
            if day_count != 0:
                human_readable_name = f'{cumsum_keyword} keyword present predominantly in last {day_count} days.'
            else:
                human_readable_name = f'{cumsum_keyword} keyword present predominantly in progress notes.'
        elif topic_match:
            topic_keyword = topic_match.groups()[0]
            human_readable_name = f'topic {topic_keyword} keyword present in progress notes.'
...
#     run only if labs data is present in the cliet data dict.
    ...
    if 'patient_alerts' in ret_dict.keys():
        alert_data = ret_dict['patient_alerts']        
        ...
        .apply(lambda x: alert_mapper(x, alert_data, x.masterpatientid, x.censusdate), axis=1)
        .values
        )
    
    processed_attributions.loc[processed_attributions.feature_type == 'Progress Note', ['human_readable_name', 'mapping_status']] = (
        processed_attributions
            .loc[processed_attributions.feature_type == 'Progress Note']
            .apply(lambda x: notes_mapper(x, x.masterpatientid, x.censusdate), axis=1)
            .values
    )
    ...
    vital_data = ret_dict['patient_vitals']
    ...
    .apply(lambda x: vital_mapper(x, vital_data, x.masterpatientid, x.censusdate), axis=1)
    .values
    )
