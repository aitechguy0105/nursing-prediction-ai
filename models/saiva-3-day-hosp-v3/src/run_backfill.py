    ...
    def execute(self):
        ...
        with start_task(action_type="run_model", client=self.client):
            ...
            for current_prediction_date in pd.date_range(
                start=self.prediction_start_date, end=self.prediction_date
            ):
                log_message(
                    message_type="info",
                    message=f"***************** Run Prediction for facility: {self.facilityid}, {current_prediction_date.date()} *****************",
                )
                ...
                pd_final = final[
                    final["censusdate"].dt.strftime("%Y-%m-%d")
                    == current_prediction_date.date().strftime("%Y-%m-%d")
                ].copy()
                log_message(
                    message_type="info",
                ...
                log_message(
                    message_type="info", num_patients_with_null_values=num_null_patients
                )
