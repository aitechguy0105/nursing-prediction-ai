...
class Ranking:
    def __init__(self, *, predictions, facilityid, facility_ml_model: FacilityMlModelConfig, client, prediction_date, saiva_engine, client_engine,
                 group_level, subclients, replace_existing_predictions, test, env=ENV, subclient_group_level='doctor'):
        self.env = env
        ...
        self.test = test
        self.subclient_group_level = subclient_group_level
    ...
    def _ranking(self, prediction_df, client, group_level):
        ...
        elif group_level != 'facility':
            # Filter the residents to be ranked subset for the (sub)client
            clientClass = get_client_class(client=client)
            residents_to_be_ranked = getattr(clientClass(), 'residents_to_be_ranked')
            prediction_df = residents_to_be_ranked(self.prediction_date, self.facilityid, self.client_sql_engine, prediction_df)
    ...
    def _rank_subclients(self):
        ...
        for subclient in self.subclients:
            ...
            prediction_df = self._ranking(
                prediction_df=self.prediction_df.copy(),
                client=subclient,
                group_level=self.subclient_group_level
            )
