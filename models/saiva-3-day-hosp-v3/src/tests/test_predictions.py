...
# python /src/run_model.py --client midwest --facilityids '[6]' --prediction_date 2021-08-15 --s3_bucket saiva-dev-data-bucket --replace_existing_predictions True --cache_s3_folder golden/run1 --save_outputs_in_s3 True --test True

# python /src/run_model.py --client uch --facilityids '[1]' --prediction_date 2021-08-15 --s3_bucket saiva-dev-data-bucket --replace_existing_predictions True --cache_s3_folder golden/run1 --save_outputs_in_s3 True --test True
...
@pytest.fixture(scope="module", params=[('midwest', '6', '2021-08-15', 'golden/run1',
                                         '/data/test/midwest/2021-08-15'),
                                         ('uch', '1', '2021-08-15', 'golden/run1',
                                         '/data/test/uch/2021-08-15')])
...
def unmatched_columns(df1, df2):
    df = pd.concat([df1,df2],axis=1)
    new_df = df.T.drop_duplicates().T
    unmatched_columns_list = [item for item, count in collections.Counter(new_df).items() if count > 1]
    print('Columns with different values are -\n')
    print(unmatched_columns_list)
...
@pytest.mark.parametrize("dataset_name", ["census", "demo", "vitals", "orders", "meds", 
                                          "alerts", "rehosp", "admissions", "diagnosis", 
                                          "notes", "finalx", "predictions"])
def test_datasets(run_model, dataset_name):
    ...
    dfs_are_equal(golden_copy_df, local_df)
...
def test_predictions_are_consistent(request):
    ...
    run1_local_path = '/data/test/midwest/2021-08-15/run1'
    run2_local_path = '/data/test/midwest/2021-08-15/run2'
    dryrun = request.config.getoption("--dryrun")
    if not dryrun:
        ...
        cmd_run_model(client = 'midwest',
                    facility_id = '6',
                    prediction_date = '2021-08-15',
