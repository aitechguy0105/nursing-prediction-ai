# """
# Run Command :
# pipenv run python /src/facility_setup.py execute --organization=beaconhc --facility_id=11
# """
...
class SetupFacility(object):
    def __init__(
        ...
        *,
        ...
        self.facility_missing_datasets = []
    ...
    def get_test_dates(
        ...
        *,
        ...
        for facility_config in self.ml_model_org_config.facilities:
            if not facility_config.trained_model:
                continue
    ...
    def feature_engineering(self):
        ...
        featurizer_dict: Dict[Type[BaseFeaturizer], Dict[str, str]] = {
            ...
            OrderFeatures: {
                'key_in': 'patient_orders',
                'name_out': 'orders_df.parquet',
            },
            MedFeatures: {
                'key_in': 'patient_meds',
                'name_out': 'meds_df.parquet',
            },
            AlertFeatures: {
                'key_in': 'patient_alerts',
                'name_out': 'alerts_df.parquet',
            },
            RehospFeatures: {
        ...
        for featurizer_class, data in featurizer_dict.items():
            facility_df = self.filter_for_facility(training_data_dict.get(data['key_in'], None))

            if facility_df.empty:
                self.facility_missing_datasets.append(data['key_in'])
                continue
        ...
        if not training_data_dict.get('patient_lab_results', pd.DataFrame()).empty:
            facility_labs_df = self.filter_for_facility(training_data_dict.get('patient_lab_results', None))

            if facility_labs_df.empty:
                self.facility_missing_datasets.append('patient_lab_results')
            else:
                labs = LabFeatures(
                            census_df=census_df.copy(),
                            labs=facility_labs_df,
                            training=True)
                labs.generate_features().to_parquet(self.processed_path/'labs_df.parquet')

        if not training_data_dict.get('patient_progress_notes', pd.DataFrame()).empty:
            facility_notes_df = self.filter_for_facility(training_data_dict.get('patient_progress_notes', None))

            if facility_notes_df.empty:
                self.facility_missing_datasets.append('patient_progress_notes')
            else:
                notes = NoteFeatures(
                    census_df=census_df.copy(),
                    notes=facility_notes_df,
                    client=self.org_id,
                    training=True
                )
    ...
    def merge_data(self) -> pd.DataFrame:
        with start_action(action_type="merge_data"):
            ...
            orders_df = pd.read_parquet(self.processed_path/'orders_df.parquet')
            alerts_df = pd.read_parquet(self.processed_path/'alerts_df.parquet')
            meds_df = pd.read_parquet(self.processed_path/'meds_df.parquet')
            ...
            final_df = demo_df.merge(vitals_df, how='left', left_on=merge_on, right_on=merge_on)\
                .merge(orders_df, how='left', left_on=merge_on, right_on=merge_on)\
                ...
                .merge(meds_df, how='left', left_on=merge_on, right_on=merge_on)\
                .merge(alerts_df, how='left', left_on=merge_on, right_on=merge_on)\
                .merge(diagnosis_df, how='left', left_on=merge_on, right_on=merge_on)

            if Path.exists(self.processed_path/'labs_df.parquet'):
                final_df = final_df.merge(
                    pd.read_parquet(self.processed_path/'labs_df.parquet'),
    ...
    def prep(self, *, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        iden_cols = ['censusdate', 'masterpatientid', 'facilityid', 'bedid', 'admissionstatus',
                     'censusactioncode', 'payername', 'payercode', 'to_from_type']
    ...
    def execute(self):
        ...
        assert best_model_id is not None, "Best model not found"
        log_message(
            message_type='info',
        ...
        facility_ml_model_config = FacilityMlModelConfigCreate(
            missing_datasets=self.facility_missing_datasets,
