...
def join_tables(result_dict):
    ...
    # Alerts are not mandotary features
    if not result_dict.get('patient_alerts', pd.DataFrame()).empty:
...
def validate_dataset(name, data):
    ...
    if name not in ['patient_lab_results']:
...
def fetch_prediction_data(
    *,
    ...
    """
    assert len(excluded_censusactioncodes) > 0, 'No excluded_censusactioncodes provided - check organization model config'
    ...
    queries = getattr(clientClass(), 'get_prediction_queries')(
        ...
        excluded_censusactioncodes=excluded_censusactioncodes
    )
    ...
    for name, query in queries.items():
        ...
        with start_action(action_type=f'pull_{name}', facilityid=facilityid):
            ...
            if result_dict[name] is None:
                ...
                log_message(message_type="info", message="saving query results to S3", path=path)
...
def fetch_training_data(
    *,
    ...
    # Dynamic import of client Class
    clientClass = get_client_class(client)
    queries = getattr(clientClass(), 'get_training_queries')(test_end_date=test_end_date, train_start_date=train_start_date, excluded_censusactioncodes=excluded_censusactioncodes)
...
def get_genric_file_names(*, data_path: str, client: str):
    """
...
def fetch_training_cache_data(client, generic=False):
    ...
    for name in get_genric_file_names(data_path=data_path, client=client):
        if generic:
    ...
    *,
    ...
    for name, query in queries.items():
        if name in model_missing_datasets:
            log_message(message_type="info", name=name, skip=f"Not fetching {name} becuase the model was not trained on it")
            continue
        with start_action(action_type=f'pull_{name}'):
            # Execute the query using pandas
            result_dict[name] = pd.read_sql(query,con=client_sql_engine)
            log_message(message_type="info", total_records=len(result_dict[name]))
