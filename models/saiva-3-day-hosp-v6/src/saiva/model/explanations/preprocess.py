...
class DataProcessor(object):
    ...
    def fetch_processed_data(self):
        ...
        if 'patient_meds' in self.raw_df_dict.keys() and len(self.raw_df_dict['patient_meds']):
        ...
        if 'patient_orders' in self.raw_df_dict.keys() and len(self.raw_df_dict['patient_orders']):
        ...
        if 'patient_alerts' in self.raw_df_dict.keys() and len(self.raw_df_dict['patient_alerts']):
        ...
        if 'patient_risks' in self.raw_df_dict.keys() and len(self.raw_df_dict['patient_risks']):
            ret_dict['patient_risks'] = self.process_risks(self.raw_df_dict['patient_risks'])
        
        if 'patient_assessments' in self.raw_df_dict.keys() and len(self.raw_df_dict['patient_assessments']):
            ret_dict['patient_assessments'] = self.process_assessments(self.raw_df_dict['patient_assessments'])
        
        if 'patient_immunizations' in self.raw_df_dict.keys() and len(self.raw_df_dict['patient_immunizations']):
            ret_dict['patient_immunizations'] = self.process_immunizations(self.raw_df_dict['patient_immunizations'])
    ...
    def fetch_filter_meds(self):
        ...
        """
        df = pd.read_csv(f'/src/saiva/static/medication_list.csv')
    ...
    def process_assessments(self, patient_assessments):
        patient_assessments['assessmentdt'] = patient_assessments['assessmentdate'].dt.date
        patient_assessments = patient_assessments.sort_values(by='assessmentdate', ascending=True)
        return patient_assessments
    
    def process_risks(self, patient_risks):
        patient_risks['incidentdt'] = patient_risks['incidentdate'].dt.date
        patient_risks = patient_risks.sort_values(by='incidentdate', ascending=True)
        return patient_risks
    
    def process_immunizations(self, patient_immunizations):
        patient_immunizations['immunizationdt'] = patient_immunizations['immunizationdate']
        patient_immunizations = patient_immunizations.sort_values(by='immunizationdate', ascending=True)
